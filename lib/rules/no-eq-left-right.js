...
module.exports = function (context) {
  ...
  return {
    ...
    "BinaryExpression": function (node) {
      ...
      if (escodegen.generate(node.left, ast.codeGenOptions) === escodegen.generate(node.right, ast.codeGenOptions)) {
    ...
    "LogicalExpression": function (node) {
      ...
      if (leftHasParts && rightHasParts) {
        var pairs = filterDuplicatesPairs(getCombinations(ast.groupNodes(node, operator).filter(hasParts).map(ast.normalizeBinaryExpression), 2));
